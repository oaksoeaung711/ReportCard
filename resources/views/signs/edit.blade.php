...
@section('title','Edit Sign')

@section('content')
    <section class="relative">
        <div class="">
            <h1 class="text-3xl text-gray-700 font-bold">Edit Sign</h1>
        </div>

        <div class="mt-10 w-1/3 p-5 shadow-lg rounded-lg">
            <form action="{{ route('signs.update',$sign->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <x-form-component formtype="text" formname="name" formid="name" formlabel="Name" formicon="signature" formplaceholder="Enter Sign Name" formvalue="{{ $sign->name }}" />

                <x-form-component formtype="text" formname="keyword" formid="keyword" formlabel="Keyword" formicon="keyboard" formplaceholder="Enter Sign Keyword" formvalue="{{ $sign->keyword }}" />
                ...
                <div class="border border-gray-300 rounded-xl p-6 relative">
                    ...
                    <div class="text-center">
                        <img id="preview" src="{{ url($sign->path) }}" class="w-24 object-cover mx-auto" />
